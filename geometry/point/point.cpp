...
struct Point {
  ...
  Point operator-(const Point &o) { return {x - o.x, y - o.y}; }
  Point operator*(const double &o) { return {x * o, y * o}; }
  bool operator==(const Point &o) { return x == o.x and y == o.y; }
  double norm2() { return x * x + y * y; }
  double norm() { return sqrt(norm2()); }
  double dot(const Point &o) { return x * o.x + y * o.y; }
  double cross(const Point &o) { return x * o.y - y * o.x; }
  double angle() {
  ...
  Point Unit() { return {x / norm(), y / norm()}; }
};
/* ========================================================= */
...
// Assumptions:
// 1) for each segment:
//  p1 should be LEFT (or COLLINEAR) and p2 should be RIGHT (or COLLINEAR) wrt
//  ray
...
bool is_si_below_sj(int i, int j) { // custom comparator based on cross product
  ...
  return (si.p1.x >= sj.p1.x) ? cross(si.p1, sj.p2, sj.p1) > 0
...
bool do_rectangles_intersect(Point &dl1, Point &ur1, Point &dl2,
                             Point &ur2) {
  return max(dl1.x, dl2.x) <= min(ur1.x, ur2.x) &&
         max(dl1.y, dl2.y) <= min(ur1.y, ur2.y);
}
/* ========================= */
...
bool do_segments_intersect(Point &p1, Point &q1, Point &p2,
                           Point &q2) {
...
double point_segment_dist(Point &p, Point &a, Point &b) {
  if (a == b)
    return (p - a).norm(); // segment is a single Point
