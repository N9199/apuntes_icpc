...
public:
  Pol(vector<T> cofs) : cofs(cofs) { this->n = cofs.size() - 1; }

  Pol<T> operator+(const Pol<T> &o) {
    vector<T> n_cofs;
    if (n > o.n) {
      n_cofs = cofs;
      rep(i, o.n + 1) { n_cofs[i] += o.cofs[i]; }
    } else {
      n_cofs = o.cofs;
      rep(i, n + 1) { n_cofs[i] += cofs[i]; }
    }
    return Pol(n_cofs);
  }
  ...
  Pol<T>
  operator*(const Pol<T> &o) // Use Fast Fourier Transform when we implement it
  {
    vector<T> n_cofs(n + o.n + 1);
    rep(i, n + 1) {
      rep(j, o.n + 1) { n_cofs[i + j] += cofs[i] * o.cofs[j]; }
    }
    return Pol(n_cofs);
  }
  ...
  Pol<T> integrate() {
    vector<T> n_cofs(n + 2);
    repx(i, 1, n_cofs.size()) { n_cofs[i] = cofs[i - 1] / T(i); }
    return Pol<T>(n_cofs);
  }
