...
struct TrieNode {
  int leaf;   // number of words that end on a TrieNode (allows for duplicate
              // words)
  int height; // height of a TrieNode, root starts at height = 1, can be changed
              // with the default value of constructor
  // number of words that pass through this node,
  // ask root node for this count to find the number of entries on the whole
  // Trie all nodes have 1 as they count the words than end on themselves (ie
  // leaf nodes count themselves)
  int count;
  TrieNode *parent; // pointer to parent TrieNode, used on erasing entries
  map<char, TrieNode *> child;
  TrieNode(TrieNode *parent = NULL, int height = 1)
      : parent(parent), leaf(0), height(height),
